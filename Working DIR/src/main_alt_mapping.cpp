// Alternate hardware mapping: Right PCA9685 RR=ch5-7, MR=ch13-15
// To use: rename to main.cpp or swap in platformio.ini
...
// LEFT  (0x40): Leg1(FL) ch 0,1,2 | Leg2(ML) ch 5,6,7 | Leg3(RL) ch 13,14,15
// RIGHT (0x41): Leg0(FR) ch 0,1,2 | Leg4(RR) ch 5,6,7 | Leg5(MR) ch 13,14,15
...
void setServoAngle(uint8_t idx, uint8_t ch, float angle) {
    int p = map((int)angle, 0, 180, SERVO_PULSE_MIN, SERVO_PULSE_MAX);
    p = constrain(p, SERVO_PULSE_MIN, SERVO_PULSE_MAX);
    pca[idx]->setPWM(ch, 0, p);
}

void setLegServos(int leg, float ca, float fa, float ta) {
    setServoAngle(LEG_CH[leg].pca_idx, LEG_CH[leg].coxa_ch,  ca);
    setServoAngle(LEG_CH[leg].pca_idx, LEG_CH[leg].femur_ch, fa);
    setServoAngle(LEG_CH[leg].pca_idx, LEG_CH[leg].tibia_ch, ta);
}
...
void leg_IK(int leg, float X, float Y, float Z) {
    ...
    switch (leg) {
        case 0: theta_coxa = constrain(theta_coxa + 45.0, 0.0, 180.0); if (leg1_IK_control) setLegServos(leg, theta_coxa, theta_femur, theta_tibia); break;
        case 1: theta_coxa = constrain(theta_coxa + 90.0, 0.0, 180.0); setLegServos(leg, theta_coxa, theta_femur, theta_tibia); break;
        case 2: theta_coxa = constrain(theta_coxa + 135.0, 0.0, 180.0); setLegServos(leg, theta_coxa, theta_femur, theta_tibia); break;
        case 3: theta_coxa = (theta_coxa < 0) ? theta_coxa + 225.0 : theta_coxa - 135.0; theta_coxa = constrain(theta_coxa, 0.0, 180.0); setLegServos(leg, theta_coxa, theta_femur, theta_tibia); break;
        case 4: theta_coxa = (theta_coxa < 0) ? theta_coxa + 270.0 : theta_coxa - 90.0; theta_coxa = constrain(theta_coxa, 0.0, 180.0); setLegServos(leg, theta_coxa, theta_femur, theta_tibia); break;
        case 5: theta_coxa = (theta_coxa < 0) ? theta_coxa + 315.0 : theta_coxa - 45.0; theta_coxa = constrain(theta_coxa, 0.0, 180.0); if (leg6_IK_control) setLegServos(leg, theta_coxa, theta_femur, theta_tibia); break;
    }
}
...
void compute_amplitudes() {
    ...
    float combined = fabs(strideX + rotOffsetX) > fabs(strideY + rotOffsetY) ? (strideX + rotOffsetX) : (strideY + rotOffsetY);
...
int calc_duration() {
    ...
    return (gait_speed == 0) ? (int)(900 - t * 650) : 1200;
}
...
void rotate_control() {
    ...
    for (leg_num = 0; leg_num < 6; leg_num++) {
        ...
        rotOffsetX = totalX*cosRy*cosRz + totalY*(sinRx*sinRy*cosRz + cosRx*sinRz) - totalZ*(cosRx*sinRy*cosRz - sinRx*sinRz) - totalX;
        rotOffsetY = -totalX*cosRy*sinRz - totalY*(sinRx*sinRy*sinRz - cosRx*cosRz) + totalZ*(cosRx*sinRy*sinRz + sinRx*cosRz) - totalY;
...
void one_leg_lift() {
    ...
    setLegServos(0, constrain(leg1_coxa + ca, 45, 135), leg1_femur, leg1_tibia);
    if (ry < -64) { int lift = map(ry, -512, -64, 24, 0); leg1_femur = constrain(leg1_femur + lift, 0, 170); leg1_tibia = constrain(leg1_tibia + 4 * lift, 0, 170); setLegServos(0, constrain(leg1_coxa + ca, 45, 135), leg1_femur, leg1_tibia); }
    ...
    setLegServos(5, constrain(leg6_coxa + ca, 45, 135), leg6_femur, leg6_tibia);
    if (ly < -64) { int lift = map(ly, -512, -64, 24, 0); leg6_femur = constrain(leg6_femur + lift, 0, 170); leg6_tibia = constrain(leg6_tibia + 4 * lift, 0, 170); setLegServos(5, constrain(leg6_coxa + ca, 45, 135), leg6_femur, leg6_tibia); }
    else if (ly > 64) { step_height_multiplier = 1.0 + (map(constrain(ly, 64, 512), 64, 512, 1, 8) - 1.0) / 3.0; }
}

void set_all_90() { for (int i = 0; i < 6; i++) setLegServos(i, 90 + COXA_CAL[i], 90 + FEMUR_CAL[i], 90 + TIBIA_CAL[i]); }
...
void setup() {
    ...
    batt_voltage_sum = 0; batt_voltage_index = 0;
